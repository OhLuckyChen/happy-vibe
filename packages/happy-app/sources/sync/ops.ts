...
export async function machineSpawnNewSession(options: SpawnSessionOptions): Promise<SpawnSessionResult> {
    ...
    const { machineId, directory, approvedNewDirectoryCreation = false, token, agent, environmentVariables, resumeClaudeSessionId, parentSessionId, forkedFromMessageId } = options;
    ...
    try {
        const result = await apiSocket.machineRPC<SpawnSessionResult, {
            ...
            machineId,
            'spawn-happy-session',
            { type: 'spawn-in-directory', directory, approvedNewDirectoryCreation, token, agent, environmentVariables, resumeClaudeSessionId, parentSessionId, forkedFromMessageId }
        );
        return result;
    } catch (error) {
        // Handle RPC errors
